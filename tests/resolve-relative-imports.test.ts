...
test("resolveFilePath should handle relative imports from subdirectories", () => {
  ...
  expect(resolveFilePath("../utils", mockFs, "components/shared/icons")).toBe(
    "components/shared/utils.ts",
  )

  // Test import from deep subdirectory to parent's parent
  expect(
    resolveFilePath("../../button", mockFs, "components/shared/icons"),
  ).toBe("components/button.tsx")

  // Test non-existent relative import
  expect(resolveFilePath("../nonexistent", mockFs, "components/shared")).toBe(
    null,
  )
})
