...
const workflowFile = join(__dirname, '..', '.github', 'workflows', 'bun-test.yml');
...
function getWorkflowTestDirectories() {
  ...
  if (!matrixMatch) {
    throw new Error('Could not find test-dir matrix configuration in workflow file');
  }
...
function main() {
  ...
  console.log(`Found ${actualTestDirs.length} test directories:`, actualTestDirs);
  console.log(`Workflow covers ${workflowTestDirs.length} directories:`, workflowTestDirs);
  
  const missing = actualTestDirs.filter(dir => !workflowTestDirs.includes(dir));
  const extra = workflowTestDirs.filter(dir => !actualTestDirs.includes(dir));
  
  if (missing.length > 0) {
    console.error('❌ ERROR: The following test directories are NOT covered by the workflow matrix:');
  ...
  if (extra.length > 0) {
    console.warn('⚠️  WARNING: The following directories in the workflow matrix do not contain test files:');
  ...
  if (missing.length === 0 && extra.length === 0) {
    console.log('✅ All test directories are properly covered by the workflow matrix!');
    process.exit(0);
  } else {
    process.exit(1);
  }
