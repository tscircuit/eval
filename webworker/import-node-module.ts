...
export const importNodeModule = async (
  ...
  if (resolvedNodeModulePath.includes("/node_modules/")) {
    const packagePath = resolvedNodeModulePath
      .split("/node_modules/")[1]
      .split("/")
    const scope = packagePath[0].startsWith("@")
    ...
    if (ctx.fsMap[packageJsonPath]) {
      try {
        ...
        if (
          packageJson.module &&
          resolvedNodeModulePath.includes(packageJson.module)
        ) {
          isFromModuleField = true
        } else if (
          packageJson.main &&
          resolvedNodeModulePath.includes(packageJson.main)
        ) {
  ...
  if (isFromModuleField) {
    ...
    preSuppliedImports[resolvedNodeModulePath.replace(/^node_modules\//, "")] =
      moduleExports
  }
